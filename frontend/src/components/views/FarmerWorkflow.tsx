...
export const FarmerWorkflow: React.FC<FarmerWorkflowProps> = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setMyOrders(ordData.items || [
        {
          ...
          participating_farmer_ids: [{ farmer_id: 'usr-farm-01', farmer_name: 'Farmer (You)', allocated_quantity_kg: 1000, price_per_kg: 25.75 }],
          match_score: 96.8,
          status: 'IN_TRANSIT',
      ...
      setNotifications(notifData.length > 0 ? notifData : [
        { id: 'notif-1', user_id: 'usr-farm-01', title: 'Buyer Match Opportunity', message: 'Reliance Retail DC needs 1,000 kg Tomato near your village.', notification_type: 'MATCH', is_read: false, created_at: '10 mins ago' },
        { id: 'notif-2', user_id: 'usr-farm-01', title: 'Pickup Route Scheduled', message: 'Vehicle MH-15-AB-4020 will collect 1,000 kg Tomato on Sept 22 at 08:30 AM.', notification_type: 'LOGISTICS', is_read: true, created_at: '2 hours ago' }
      ]);
    } catch (err) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="glass-panel" style={{
        background: 'linear-gradient(135deg, rgba(16,185,129,0.16) 0%, rgba(6,182,212,0.14) 100%)',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <button 
            ...
            style={{ fontSize: '1.05rem', padding: '14px 26px', boxShadow: '0 6px 22px rgba(16,185,129,0.45)' }}
          >
            <PlusCircle size={22} />
            {t.farmer.addCropBtn}
          </button>
      ...
      {userFriendlyMsg && (
        <div style={{
          ...
          <button 
            ...
            style={{ background: 'transparent', border: 'none', color: '#cbd5e1', cursor: 'pointer', fontWeight: 700 }}
          >
            ✕
          </button>
        </div>
      ...
      <div style={{
        ...
        <div className="glass-panel" style={{ borderLeft: '5px solid #10b981' }}>
          ...
          <div style={{ fontSize: '0.82rem', color: '#94a3b8', marginTop: '2px' }}>
            {mySupplies.length} {language === 'ta' ? 'செயலில் உள்ள பயிர்கள்' : language === 'hi' ? 'सक्रिय फसलें' : language === 'te' ? 'క్రియాశీల పంటలు' : language === 'ml' ? 'സജീവ വിളകൾ' : language === 'kn' ? 'ಸಕ್ರಿಯ ಬೆಳೆಗಳು' : 'active listings'}
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ borderLeft: '5px solid #38bdf8' }}>
          ...
          <div style={{ fontSize: '1.9rem', fontWeight: 800, color: '#38bdf8', marginTop: '4px' }}>
            {demandOpps.length} {language === 'ta' ? 'தேவைகள்' : language === 'hi' ? 'मांगें' : language === 'te' ? 'డిమాండ్లు' : language === 'ml' ? 'ആവശ്യങ്ങൾ' : language === 'kn' ? 'ಬೇಡಿಕೆಗಳು' : 'demands'}
          </div>
          <div style={{ fontSize: '0.82rem', color: '#94a3b8', marginTop: '2px' }}>
            {language === 'ta' ? 'அருகிலுள்ள நிறுவன வாங்குபவர்கள்' : language === 'hi' ? 'सत्यापित संस्थागत खरीदार' : language === 'te' ? 'ధృవీకరించబడిన సంస్థాగత కొనుగోలుదారులు' : language === 'ml' ? 'സ്ഥിരീകരിച്ച ഇൻസ്റ്റിറ്റ്യൂഷണൽ വ്യാപാരികൾ' : language === 'kn' ? 'ಪರಿಶೀಲಿಸಿದ ಸಾಂಸ್ಥಿಕ ಖರೀದಿದಾರರು' : 'verified institutional buyers'}
          </div>
        </div>

        {/* Available Harvest Stock */}
        <div className="glass-panel" style={{ borderLeft: '5px solid #a855f7' }}>
          <span style={{ fontSize: '0.82rem', color: '#cbd5e1', fontWeight: 600, textTransform: 'uppercase' }}>
            📦 {t.farmer.summary.availableStock}
          </span>
          <div style={{ fontSize: '1.9rem', fontWeight: 800, color: '#c084fc', marginTop: '4px' }}>
        ...
        <div className="glass-panel" style={{ borderLeft: '5px solid #06b6d4' }}>
          ...
          <div style={{ fontSize: '1.9rem', fontWeight: 800, color: '#38bdf8', marginTop: '4px' }}>
            {myOrders.length} {language === 'ta' ? 'ஆர்டர்கள்' : language === 'hi' ? 'सक्रिय ऑर्डर' : language === 'te' ? 'క్రియాశీల ఆర్డర్లు' : language === 'ml' ? 'സജീവ ഓർഡറുകൾ' : language === 'kn' ? 'ಸಕ್ರಿಯ ಆದೇಶಗಳು' : 'active'}
          </div>
          <div style={{ fontSize: '0.82rem', color: '#94a3b8', marginTop: '2px' }}>
      ...
      {activeTab === 'dashboard' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          <div>
            <h2 style={{ fontSize: '1.15rem', fontWeight: 700, marginBottom: '14px', color: '#cbd5e1' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
              <div className="action-card" onClick={() => handleOpenCropWizard('EXPECTED')} style={{ borderLeft: '4px solid #10b981' }}>
                ...
                <div>
                  <div style={{ fontWeight: 800, fontSize: '1.05rem', color: '#10b981' }}>{t.farmer.quickActions.postStock}</div>
                  <div style={{ fontSize: '0.82rem', color: '#94a3b8', marginTop: '2px' }}>{t.farmer.quickActions.postStockSub}</div>
                </div>
              </div>
              ...
              <div className="action-card" onClick={() => setActiveTab('demands')} style={{ borderLeft: '4px solid #38bdf8' }}>
                <div style={{ fontSize: '1.8rem' }}>📊</div>
                <div>
                  <div style={{ fontWeight: 800, fontSize: '1.05rem', color: '#38bdf8' }}>{t.farmer.quickActions.viewDemand}</div>
                  <div style={{ fontSize: '0.82rem', color: '#94a3b8', marginTop: '2px' }}>{t.farmer.quickActions.viewDemandSub}</div>
                </div>
              </div>

              <div className="action-card" onClick={() => handleOpenCropWizard('STOCK')} style={{ borderLeft: '4px solid #a855f7' }}>
                <div style={{ fontSize: '1.8rem' }}>📦</div>
                <div>
                  <div style={{ fontWeight: 800, fontSize: '1.05rem', color: '#c084fc' }}>
                    {language === 'ta' ? 'அறுவடை இருப்பு பதிவு' : language === 'hi' ? 'बिक्री स्टॉक पोस्ट करें' : language === 'te' ? 'అమ్మకం కాని స్టాక్ పోస్ట్ చేయండి' : language === 'ml' ? 'വിൽക്കാത്ത സ്റ്റോക്ക് പോസ്റ്റ് ചെയ്യുക' : language === 'kn' ? 'ದಾಸ್ತಾನು ಪೋಸ್ಟ್ ಮಾಡಿ' : 'Post Unsold Stock'}
                  </div>
                  <div style={{ fontSize: '0.82rem', color: '#94a3b8', marginTop: '2px' }}>
                    {language === 'ta' ? 'உடனடி வாங்குபவர்களைக் கண்டறிய' : language === 'hi' ? 'आपातकालीन खरीदार खोजें' : language === 'te' ? 'అత్యవసర కొనుగోలుదారులను కనుగొనండి' : language === 'ml' ? 'അടിയന്തര വ്യാപാരികളെ കണ്ടെത്തുക' : language === 'kn' ? 'ತುರ್ತು ಖರೀದಿದಾರರನ್ನು ಹುಡುಕಿ' : 'Find nearby emergency buyers'}
                  </div>
                </div>
              </div>

              <div className="action-card" onClick={() => setActiveTab('orders')} style={{ borderLeft: '4px solid #f59e0b' }}>
                <div style={{ fontSize: '1.8rem' }}>🚚</div>
                <div>
                  <div style={{ fontWeight: 800, fontSize: '1.05rem', color: '#fbbf24' }}>{t.farmer.quickActions.trackOrders}</div>
                  <div style={{ fontSize: '0.82rem', color: '#94a3b8', marginTop: '2px' }}>{t.farmer.quickActions.trackOrdersSub}</div>
                </div>
              </div>
            </div>
          </div>

          {/* Active Pre-Harvest Declarations List */}
          <div className="glass-panel">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              >
                + {language === 'ta' ? 'பயிர் சேர்க்க' : language === 'hi' ? 'फसल जोड़ें' : language === 'te' ? 'పంటను జోడించండి' : language === 'ml' ? 'വിള ചേർക്കുക' : language === 'kn' ? 'ಬೆಳೆ ಸೇರಿಸಿ' : 'Add Crop'}
              </button>
            </div>
            ...
            {mySupplies.length > 0 ? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '14px' }}>
                {mySupplies.map(sup => {
                  const cropName = crops.find(c => c.id === sup.crop_id)?.name || 'Tomato';
                  return (
                    <div key={sup.id} className="surface-card" style={{ borderLeft: '4px solid #10b981' }}>
                      ...
                      <div style={{ fontSize: '0.88rem', color: '#cbd5e1', marginTop: '6px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <div>Expected Quantity: <strong style={{ color: '#fff' }}>{sup.expected_quantity_kg.toLocaleString('en-IN')} kg</strong></div>
                        <div>Harvest Date: <strong style={{ color: '#38bdf8' }}>{sup.expected_harvest_date}</strong></div>
                        <div>Quality Grade: <span>{sup.quality_grade}</span></div>
                      </div>
                    </div>
                  );
                })}
                  ...
                  <div key={ord.id} className="surface-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <strong style={{ color: '#f8fafc', fontSize: '1rem' }}>Order #{ord.id}</strong>
                      </div>
                      <div style={{ fontSize: '0.85rem', color: '#94a3b8', marginTop: '4px' }}>
                        Quantity: <strong>{ord.total_matched_quantity_kg} kg</strong> | Buyer Payout: <strong style={{ color: '#10b981' }}>₹{ord.agreed_farmer_price_per_kg}/kg</strong>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: '1.35rem', fontWeight: 800, color: '#fbbf24' }}>
                        ₹{ord.total_amount_inr?.toLocaleString('en-IN')}
                      </div>
                      <div style={{ fontSize: '0.8rem', color: '#34d399' }}>Guaranteed Direct Transfer</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="glass-panel">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
            ...
            <button className="btn-emerald" onClick={() => handleOpenCropWizard('EXPECTED')}>
              <PlusCircle size={18} /> {t.farmer.addCropBtn}
            </button>
          </div>
              ...
              return (
                <div key={sup.id} className="surface-card" style={{ borderLeft: '4px solid #10b981' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <StatusBadge status={sup.status} />
                    <span style={{ fontSize: '1.25rem', fontWeight: 800, color: '#10b981' }}>₹{sup.min_price_per_kg}/kg</span>
                  </div>
                  <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: '#ffffff' }}>
                    {cropIcons[cropName] || '🌾'} {cropName}
                  </h3>
                  <div style={{ fontSize: '0.88rem', color: '#cbd5e1', marginTop: '6px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <div>Quantity: <strong>{sup.expected_quantity_kg.toLocaleString('en-IN')} kg</strong></div>
                    <div>Target Date: <strong style={{ color: '#38bdf8' }}>{sup.expected_harvest_date}</strong></div>
                    <div>Grade: <strong>{sup.quality_grade}</strong></div>
                  </div>
                </div>
              );
            })}
        ...
        <div className="glass-panel">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
            ...
            <button className="btn-emerald" onClick={() => handleOpenCropWizard('STOCK')}>
              <PlusCircle size={18} /> {language === 'ta' ? 'கையிருப்பு சேர்க்க' : language === 'hi' ? 'स्टॉक लिस्टिंग जोड़ें' : language === 'te' ? 'స్టాక్ లిస్టింగ్ జోడించండి' : language === 'ml' ? 'സ്റ്റോക്ക് ലിസ്റ്റിംഗ് ചേർക്കുക' : language === 'kn' ? 'ದಾಸ್ತಾನು ಪಟ್ಟಿ ಸೇರಿಸಿ' : 'Add Stock Listing'}
            </button>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
            {myStocks.map(stk => {
              const cropName = crops.find(c => c.id === stk.crop_id)?.name || 'Tomato';
              return (
                <div key={stk.id} className="surface-card" style={{ borderLeft: '4px solid #a855f7' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <StatusBadge status={stk.status} />
                    <span style={{ fontSize: '1.25rem', fontWeight: 800, color: '#c084fc' }}>₹{stk.price_per_kg}/kg</span>
                  </div>
                  <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: '#ffffff' }}>
                    {cropIcons[cropName] || '🌾'} {cropName}
                  </h3>
                  <div style={{ fontSize: '0.88rem', color: '#cbd5e1', marginTop: '6px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <div>Available Stock: <strong>{stk.available_quantity_kg.toLocaleString('en-IN')} kg</strong></div>
                    <div>Harvested On: <strong>{stk.harvest_date}</strong></div>
                    <div>Remaining Shelf Life: <strong style={{ color: '#fbbf24' }}>{stk.shelf_life_remaining_days} days</strong></div>
                  </div>
                </div>
              );
            })}
      ...
      {/* ==================================================== */}
      {/* 7. BUYER DEMAND OPPORTUNITIES TAB */}
      {/* ==================================================== */}
      {activeTab === 'demands' && (
        <div className="glass-panel">
          <h2 style={{ fontSize: '1.3rem', fontWeight: 800, marginBottom: '16px', color: '#f8fafc' }}>
            🤝 {t.farmer.tabs.demands}
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px' }}>
            {demandOpps.map(dem => (
              <div key={dem.id} className="surface-card" style={{ borderLeft: '4px solid #38bdf8' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <StatusBadge status="HIGH_DEMAND" />
                  <span style={{ fontSize: '1.3rem', fontWeight: 800, color: '#10b981' }}>₹{dem.max_price_per_kg}/kg</span>
                </div>
                ...
                <div style={{ fontSize: '0.88rem', color: '#cbd5e1', marginTop: '6px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  ...
                  <div>Needed By: <strong>{dem.target_delivery_date}</strong></div>
                  <div>Destination: <span>{dem.delivery_address}</span></div>
                </div>
                <button 
                  className="btn-emerald" 
                  onClick={() => handleOpenCropWizard('EXPECTED')}
                  style={{ marginTop: '14px', width: '100%', minHeight: '42px', fontSize: '0.9rem' }}
                >
                  {language === 'ta' ? 'இந்தத் தேவைக்கு பயிர் ஒதுக்குக' : language === 'hi' ? 'इस मांग के लिए आपूर्ति प्रतिबद्ध करें' : language === 'te' ? 'ఈ డిమాండ్ కోసం సరఫరాను కేటాయించండి' : language === 'ml' ? 'ഈ ആവശ്യത്തിന് വിള നൽകുക' : language === 'kn' ? 'ಈ ಬೇಡಿಕೆಗೆ ಬೆಳೆ ನಿಯೋಜಿಸಿ' : 'Commit Supply to this Demand'}
                </button>
              </div>
            ))}
          </div>
      ...
      {activeTab === 'orders' && (
        <div className="glass-panel">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {myOrders.map(ord => (
              <div key={ord.id} className="surface-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '14px' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <StatusBadge status={ord.status} />
                    <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#f8fafc' }}>Order #{ord.id}</h3>
                  </div>
                  <div style={{ fontSize: '0.88rem', color: '#cbd5e1', marginTop: '6px' }}>
                    Crop: <strong>Tomato</strong> | Quantity: <strong>{ord.total_matched_quantity_kg} kg</strong>
                  </div>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: '0.82rem', color: '#34d399', fontWeight: 700 }}>
                    ₹{ord.agreed_farmer_price_per_kg}/kg Direct Payout
                  </div>
                </div>
              </div>
            ))}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {notifications.map(n => (
              <div 
                ...
                style={{
                  borderLeft: n.is_read ? '4px solid #475569' : '4px solid #10b981',
                  background: n.is_read ? 'rgba(255,255,255,0.02)' : 'rgba(16,185,129,0.06)',
                  cursor: 'pointer'
                }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontWeight: 700, fontSize: '0.98rem', color: n.is_read ? '#cbd5e1' : '#ffffff' }}>
                    {n.title}
                  </div>
                  <span style={{ fontSize: '0.78rem', color: '#94a3b8' }}>{n.created_at || 'Just now'}</span>
                </div>
      ...
      {/* ==================================================== */}
      {/* 10. SIMPLIFIED 5-STEP CROP ENTRY WIZARD MODAL */}
      {/* ==================================================== */}
      <Modal
        isOpen={showCropWizard}
        onClose={() => setShowCropWizard(false)}
        title={wizardMode === 'EXPECTED' ? t.farmer.wizard.title : (language === 'ta' ? 'அறுவடை செய்த கையிருப்பு பதிவு' : language === 'hi' ? 'कटाई स्टॉक लिस्टिंग' : language === 'te' ? 'కోత స్టాక్ లిస్టింగ్' : language === 'ml' ? 'വിളവെടുത്ത സ്റ്റോക്ക് ലിസ്റ്റിംഗ്' : language === 'kn' ? 'ಕೊಯ್ಲು ಮಾಡಿದ ದಾಸ್ತಾನು ಪಟ್ಟಿ' : 'Post Harvested Stock')}
        maxWidth="620px"
      >
        <div>
          {/* Stepper Dots */}
          <div className="stepper-container" style={{ marginBottom: '20px' }}>
            {[1, 2, 3, 4, 5].map((stepNum) => {
              ...
              return (
                <div key={stepNum} className="stepper-step">
                  <div className={`stepper-circle ${isActive ? 'active' : isDone ? 'completed' : ''}`}>
                    {isDone ? '✓' : stepNum}
                  </div>
                  <span style={{ fontSize: '0.72rem', color: isActive ? '#34d399' : '#94a3b8' }}>
                    {stepNum === 1 ? t.farmer.wizard.step1 :
                     stepNum === 2 ? t.farmer.wizard.step2 :
                     stepNum === 3 ? t.farmer.wizard.step3 :
                     stepNum === 4 ? t.farmer.wizard.step4 : t.farmer.wizard.step5}
                  </span>
                </div>
              );
            })}
          ...
          {/* Form Error Banner */}
          {formError && (
            <div style={{ background: 'rgba(239, 68, 68, 0.15)', border: '1px solid #ef4444', color: '#f87171', padding: '10px 14px', borderRadius: '10px', fontSize: '0.88rem', marginBottom: '16px' }}>
              ⚠️ {formError}
            </div>
          )}
                ...
                {crops.map(c => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ fontWeight: 800, fontSize: '0.95rem', color: isSelected ? '#34d399' : '#f8fafc' }}>
                        {c.name}
                      </div>
                      <div style={{ fontSize: '0.78rem', color: '#94a3b8', marginTop: '2px' }}>
                        Base: ₹{c.indicative_base_price_per_kg}/kg
                      </div>
                    </div>
                  );
                })}
          ...
          {wizardStep === 2 && (
            <div>
              ...
              <label className="input-label">{t.farmer.wizard.quantityLabel}</label>
              <input
                type="number"
                value={qtyKg}
              ...
              <div style={{ marginTop: '14px' }}>
                <span style={{ fontSize: '0.82rem', color: '#94a3b8', fontWeight: 600 }}>{t.farmer.wizard.step2Presets}</span>
                  ...
                  {['500', '1000', '2500', '5000', '10000'].map(p => (
                    <button
                      ...
                      style={{
                        padding: '6px 14px',
                        borderRadius: '8px',
                        border: qtyKg === p ? '1px solid #10b981' : '1px solid rgba(255,255,255,0.1)',
                        background: qtyKg === p ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.04)',
                        color: qtyKg === p ? '#34d399' : '#cbd5e1',
                        cursor: 'pointer',
                        fontWeight: 700,
                        fontSize: '0.85rem'
                      }}
                    >
                      {parseInt(p).toLocaleString('en-IN')} kg
                    </button>
                  ))}
                </div>
              </div>
          ...
          {wizardStep === 3 && (
            <div>
              ...
              <label className="input-label">{t.farmer.wizard.priceLabel}</label>
              <input
                type="number"
                step="0.5"
                value={pricePerKg}
                ...
                className="input-large"
                style={{ fontSize: '1.4rem', fontWeight: 800, color: '#fbbf24' }}
                placeholder="e.g. 24.50"
              />

              <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', padding: '12px 14px', borderRadius: '10px', marginTop: '14px', fontSize: '0.84rem', color: '#fbbf24' }}>
                💡 <strong>Advisory Market Guidance:</strong> Current Nashik district buyer willingness for {selectedCropName} ranges from <strong>₹24.00 - ₹28.50/kg</strong>.
              </div>
            </div>
          )}
          ...
          {wizardStep === 4 && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div>
                  <label className="input-label">{t.farmer.wizard.dateLabel}</label>
                  <input
                    type="date"
                    value={targetDate}
                ...
                <div>
                  <label className="input-label">{t.farmer.wizard.districtLabel}</label>
                  <select
                    value={district}
                ...
                <div>
                  <label className="input-label">{t.farmer.wizard.gradeLabel}</label>
                  <select
                    value={grade}
                    onChange={(e) => setGrade(e.target.value as any)}
                    className="input-large"
                  >
                    <option value="GRADE_A">Grade A (Standard Market Quality)</option>
                    <option value="ORGANIC">Certified Organic</option>
                    <option value="EXPORT">Export Quality</option>
                    <option value="GRADE_B">Grade B (Processing / Puree Quality)</option>
                  </select>
                </div>
              </div>
            </div>
          ...
          {wizardStep === 5 && (
            <div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '14px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '8px' }}>
                  <span style={{ color: '#94a3b8' }}>{t.farmer.wizard.cropLabel}:</span>
                  <strong style={{ color: '#ffffff', fontSize: '1.05rem' }}>{selectedCropName}</strong>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '8px' }}>
                  <span style={{ color: '#94a3b8' }}>{t.farmer.wizard.quantityLabel}:</span>
                  <strong style={{ color: '#10b981', fontSize: '1.1rem' }}>{parseFloat(qtyKg).toLocaleString('en-IN')} kg</strong>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '8px' }}>
                  <span style={{ color: '#94a3b8' }}>{t.farmer.wizard.priceLabel}:</span>
                  <strong style={{ color: '#fbbf24', fontSize: '1.1rem' }}>₹{pricePerKg}/kg</strong>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '8px' }}>
                  <span style={{ color: '#94a3b8' }}>{t.farmer.wizard.dateLabel}:</span>
                  <strong style={{ color: '#38bdf8' }}>{targetDate}</strong>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#94a3b8' }}>Estimated Payout Realization:</span>
                  <strong style={{ color: '#34d399', fontSize: '1.2rem' }}>
                    ₹{(parseFloat(qtyKg || '0') * parseFloat(pricePerKg || '0')).toLocaleString('en-IN')}
                  </strong>
                </div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '24px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
            {wizardStep > 1 ? (
              <button className="btn-secondary" onClick={handleStepBack} disabled={isSubmitting}>
                <ArrowLeft size={16} /> {t.common.back}
              </button>
            ) : (
              <button className="btn-secondary" onClick={() => setShowCropWizard(false)} disabled={isSubmitting}>
                {t.common.cancel}
              </button>
            )}

            {wizardStep < 5 ? (
              <button className="btn-emerald" onClick={handleStepNext}>
              ...
              >
                {isSubmitting ? (
                  ...
                  <span>{wizardMode === 'EXPECTED' ? t.farmer.wizard.publishExpectedBtn : t.farmer.wizard.publishStockBtn}</span>
                )}
              </button>
            )}
          </div>
